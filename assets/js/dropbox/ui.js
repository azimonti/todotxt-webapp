...
export function updateSyncIndicator(status, message = '') {
  ...
  // Avoid unnecessary updates if status hasn't changed (unless it's an error message)
  if (status === currentSyncStatus && status !== SyncStatus.ERROR) return;

  logVerbose(`Updating sync indicator: ${status}`, message || '');
  ...
  case SyncStatus.IDLE:
    ...
    text = ''; // or 'Synced'
    title = `Synced ${new Date().toLocaleTimeString()}`;
    break;
...
export function showConflictModal(localDate, dropboxDate) {
  ...
  logVerbose('Showing conflict modal.');
  // Show the modal
